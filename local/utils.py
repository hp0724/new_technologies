...
def draw_scenes(points, gt_boxes=None, ref_boxes=None, ref_scores=None, ref_labels=None):
    ...
    for i in range(len(ref_boxes)):
        if(ref_scores[count] < 0.4):
            points = np.delete(points,count,axis=0)
            ref_boxes = np.delete(ref_boxes, count, axis=0)
            ref_scores = np.delete(ref_scores, count, axis=0)
            ref_labels = np.delete(ref_labels, count, axis=0)
        else :
            count = count + 1
